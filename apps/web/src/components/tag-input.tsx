    ...
    <div className="flex w-[300px] flex-col md:w-[400px]">
      ...
      <div className="mt-4 flex flex-row flex-wrap gap-3">
        {tags.map((tag: string, index: number) => (
          <span
            key={`${index}-${tag}`}
            className="bg-muted mr-2 flex items-center justify-end gap-2 rounded-[32px] p-1 pl-3 text-sm font-medium text-primary shadow-sm"
          >
            {tag}
            <button
              className="bg-muted/40 flex size-5 items-center justify-center rounded-full text-white hover:bg-primary"
